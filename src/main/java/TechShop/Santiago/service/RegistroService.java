...
public class RegistroService {
    ...
    public RegistroService(
        ...
        Optional<Constante> constante =
                constanteService.findByAtributo("servidor.http");

        servidor = constante.isPresent()
                ? constante.get().getValor()
                : "localhost";
    }
    ...
    public Model recordarUsuario(Model model, Usuario usuario)
        ...
        Optional<Usuario> usuarioOpt =
                usuarioService.getUsuarioPorUsernameOCorreo(
                        usuario.getUsername(),
                        usuario.getCorreo()
                );
    ...
    private String demeClave() {
        ...
        String tira =
                "ABCDEFGHIJKLMNOPQRSTUXYZabcdefghijklmnopqrstuvwxyz0123456789";

        String clave = "";

        for (int i = 0; i < 40; i++) {
            clave += tira.charAt(
                    (int) (Math.random() * tira.length())
            );
        }
    ...
    private void enviaCorreoActivar(
            Usuario usuario,
            String clave)
            throws MessagingException {
    ...
    private void enviaCorreoRecordar(
            Usuario usuario,
            String clave)
            throws MessagingException {
